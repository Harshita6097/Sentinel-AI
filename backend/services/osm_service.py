...
def get_osm_graph():
    ...
    try:
        ...
        for name, (lat, lng) in _LOCATION_COORDS.items():
            try:
                nmap[name] = ox.nearest_nodes(G, lng, lat)
            except Exception:
                pass
...
def osm_shortest_path(origin: str, destination: str) -> dict | None:
    """Compute shortest path between two named Kerala locations using OSM graph.

    Returns dict with route, total_time_minutes, reachable — or None if unavailable.
    """
    G, node_map = get_osm_graph()
    if G is None or origin not in node_map or destination not in node_map:
        return None
    try:
        import networkx as nx

        src, dst = node_map[origin], node_map[destination]
        ...
        named_route = [reverse_map[n] for n in path_nodes if n in reverse_map]
