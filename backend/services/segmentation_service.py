...
ADE20K class indices used for flood proxy:
  ...
  - 9:  road   — used to compute road submersion ratio
"""
import hashlib
import io
import logging
...
# ADE20K class IDs that indicate flood / water presence
_WATER_CLASSES = {21, 26, 60, 128}  # water, sea, river, lake
...
def _mock_segmentation(image_bytes: bytes) -> SegmentationResult:
    """Deterministic mock segmentation from image content hash."""
...
def _real_segmentation(image_bytes: bytes, model, processor) -> SegmentationResult:
    """Run real SegFormer inference and map ADE20K classes to flood metrics."""
    try:
        import torch
        import numpy as np
        ...
        water_pixels = sum(
            int(np.sum(seg_map == cls)) for cls in _WATER_CLASSES
        )
