...
Semantic similarity compares the normalised report text against stored incident
texts using cosine similarity on sentence embeddings.
"""
import logging
...
SEMANTIC_THRESHOLD = 0.82   # cosine similarity threshold for semantic match
...
def check(
    ...
    Args:
        ...
        raw_text:           Original report text for semantic comparison.
...
def _semantic_check(
    ...
    """Use SentenceTransformer cosine similarity. Returns None if model unavailable."""
    try:
...
def _keyword_check(
    ...
    """Weighted keyword similarity fallback."""
    best_score = 0.0
