...
def test_cop_initial_state(client):
    r = client.get("/api/commander/cop")
...
def test_recommendations_have_evidence_sources(client):
    client.post("/api/emergency/report", json={
        "text": "Flood in Kottayam. Hospital at risk."
    })
    r = client.post("/api/commander/recompute")
    recs = r.json()["recommendations"]
    if recs:
        assert "evidence_sources" in recs[0]
...
def test_decision_log_grows(client):
    ...
    client.post("/api/commander/recompute")
    r = client.get("/api/commander/decision-log")
    assert r.status_code == 200
    log = r.json()
    assert len(log) >= 2
...
def test_commander_reset(client):
    ...
    client.post("/api/commander/recompute")
    r = client.post("/api/commander/reset")
    assert r.status_code == 200
    log = client.get("/api/commander/decision-log").json()
    assert len(log) == 0
...
def test_priority_engine_scores_location():
    ...
    result = score_location("Alappuzha", cop)
    assert result.score > 50
    assert result.location == "Alappuzha"
...
def test_evidence_fusion_all_sources():
    ...
    assert result.dominant_source == "logistics"


def test_evidence_fusion_partial():
    from services.evidence_fusion import fuse
    result = fuse({"emergency": 85})
