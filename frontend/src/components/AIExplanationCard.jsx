...
export default function AIExplanationCard({ triggerKey }) {
  const [data,    setData]    = useState(null)
  const [loading, setLoading] = useState(false)

  useEffect(() => {
    setLoading(true)
    api.explainDecision(0)
      .then(setData)
      .catch(() => {})
  ...
  return (
    <div style={s.card}>
      <div style={s.header}>
        ...
        <span style={s.title}>Commander Explanation</span>
        {data && (
      ...
      {loading ? (
        ...
        <div style={s.muted}>No recommendation to explain yet</div>
      )}
    </div>
  )
}
